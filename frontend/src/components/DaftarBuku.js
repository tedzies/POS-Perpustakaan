...
function DaftarBuku() {
  ...
  const [judulBaru, setJudulBaru] = useState("Default Value");
  const [penerbitBaru, setPenerbitBaru] = useState("Default Value");
  const [pengarangBaru, setPengarangBaru] = useState();

  const getBuku = () => {
    Axios.get("http://localhost:3001/daftar").then((response) => {
